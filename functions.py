...
def baselease_addons(base_lease, conds, start_date, forecasting_period):
    ...
    prev_loc_rates = pd.DataFrame({'Location': ['Colocation R1 Grid', 'Colocation R2 Grid', 'Colocation R3 Grid', 
    ...
    if base_lease.shape[0]==0:
        base_lease = prev_loc_rates.copy()
        base_lease.rename(columns = {'months':'Change Months'}, inplace=True)
        
    locs = prev_loc_rates.Location.unique()
    ...
    for k in locs:
        if k in changed_locs:
            ...
            temp = prev_loc_rates[prev_loc_rates.Location == k]
    ...
    for p in list(unchanged_locs):
        df2 = prev_loc_rates[prev_loc_rates.Location.isin([p])].merge(months, how = 'right', on = 'months')
        temp = prev_loc_rates[prev_loc_rates.Location == k]
        df2 = temp.append(df, ignore_index=True)
    ...
    loc_tab['rate'] = loc_tab['Base Rate'] * (1 + (loc_tab['Addon Rate']/100))
